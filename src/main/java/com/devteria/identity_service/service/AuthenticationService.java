...
public class AuthenticationService {
    ...
    public AuthenticationResponse refreshToken(RefreshRequest request) throws ParseException, JOSEException {
        ...
        var user = userRepository.findByUsername(username).orElseThrow(() -> new AppException(ErrorCode.USER_NOT_EXIT));
        var token = generateToken(user);
        return AuthenticationResponse.builder().token(token).success(true).build();
    }
    ...
    private SignedJWT verifyToken(String token, boolean isRefresh) throws ParseException, JOSEException {
        ...
        if (!(verified && expiration.after(new Date()))) throw new AppException(ErrorCode.AUTHENTICATED);
    ...
    private String buildCode(User us) {
        StringJoiner stringJoiner = new StringJoiner(" ");
        if (!CollectionUtils.isEmpty(us.getRoles())) {
            us.getRoles().forEach(role -> {
                stringJoiner.add("ROLE_" + role.getName());
                if (!CollectionUtils.isEmpty(role.getPermissionSet()))
                    role.getPermissionSet().forEach(permission -> stringJoiner.add(permission.getName()));
            });
        }
